...
export const cycloid: AnimationDef = {
  ...
  category: 'Geometric',
  name: 'Cycloid',
  tag: 'Rolling Wheel Trace',
  params: [
    // --- Curve geometry ---
    { key: 'r', label: 'Wheel Radius', labelZh: '滚轮半径', type: 'range', min: 3, max: 15, step: 0.5, val: 8 },
    ...
    { key: 'durationMs', label: 'Duration (ms)', labelZh: '周期时长', type: 'range', min: 2000, max: 15000, step: 100, val: 8000 },
    // --- Path appearance ---
    { key: 'strokeWidth', label: 'Stroke Width', labelZh: '描边宽度', type: 'range', min: 0.5, max: 10, step: 0.1, val: 3 },
    { key: 'pathOpacity', label: 'Path Opacity', labelZh: '路径透明度', type: 'range', min: 0, max: 1, step: 0.05, val: 0.5 },
    // --- Particles ---
    ...
    // --- Static fallback color ---
    { key: 'color', label: 'Static Color', labelZh: '静态颜色', type: 'color', val: '#20b2aa' },
  ],
  formula(cfg) {
    return [
      `x(t) = r(t - sin(t))`,
  ...
  point(progress, _time, cfg) {
    ...
    // 3 full arches of the cycloid
    const t = progress * Math.PI * 6;
    const x = r * (t - Math.sin(t));
    const y = r * (1 - Math.cos(t));
    // Center the curve horizontally: midpoint of 3 arches is at x = r*3*pi
    const offset = r * 3 * Math.PI;
    return {
      x: 50 + (x - offset) * scale,
      y: 50 - y * scale,
    };
  ...
  code(cfg) {
    return `const r = ${(cfg.r as number).toFixed(1)};
